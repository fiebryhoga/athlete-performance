...
import "axios";
function Edit({ packageData, exercises = [] }) {
  const {
    data,
    setData,
    put,
    ...
    exercise_ids: packageData.exercises ? packageData.exercises.map((ex) => ex.id) : []
  });
  const [searchPackageEx, setSearchPackageEx] = useState("");
  const submit = (e) => {
    e.preventDefault();
    put(route("admin.exercise-packages.update", packageData.id));
  };
  ...
  return /* @__PURE__ */ jsxs(AppLayout, { title: "Edit Paket Latihan", children: [
    /* @__PURE__ */ jsx(Head, { title: `Edit Paket Latihan - ${packageData.name}` }),
    /* @__PURE__ */ jsxs("div", { className: "mb-8", children: [
      ...
      /* @__PURE__ */ jsx("p", { className: "text-gray-600 mt-2", children: "Ubah informasi dan daftar latihan dalam paket ini." })
    ] }),
