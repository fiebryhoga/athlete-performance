...
export default function Index({ athletes, groups }) {
    ...
    return (
        <AppLayout title="Recovery Strategi">
            <Head title="Recovery Strategi" />
            <PageHeader 
                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            {filteredAthletes.map(athlete => (
                                <Link key={athlete.id} href={route('admin.recovery-strategies.show', athlete.id)} className="block group">
                                        ...
                                        <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between relative z-10">
                                            ...
                                            {athlete.sport && (
                                                <div className="text-right">
                                                    <p className="text-[10px] font-bold text-slate-400">Olahraga</p>
                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            {filteredGroups.map(group => (
                                <Link key={group.id} href={route('admin.recovery-strategies.group.show', group.id)} className="block group">
                                    <div className="bg-white border border-slate-200 hover:border-slate-900 hover:shadow-md rounded-xl p-5 transition-all shadow-sm flex flex-col h-full relative overflow-hidden group-hover:-translate-y-1">
                                        <div className="absolute -right-4 -top-4 w-24 h-24 bg-slate-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-500 pointer-events-none"></div>
                                        <div className="flex justify-between items-start mb-4 relative z-10">
                                            <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200 ">
                                        ...
                                        <div className="mt-auto pt-4 border-t border-slate-100 relative z-10">
                                            ...
                                            {group.members?.length > 0 && (
                                                <div className="pt-3 border-t border-slate-50 flex flex-wrap gap-1">
                                                    {group.members.map(member => (
                                                        <span key={member.id} className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium bg-slate-50 border border-slate-200 text-slate-600">
                                                            {member.name}
                                                        </span>
                                                    ))}
                                                </div>
