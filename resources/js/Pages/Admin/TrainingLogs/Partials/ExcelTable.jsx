...
export default function ExcelTable({ data, is_athlete, handleExChange, libExercises, handleAddNewExercise, addNewRow, removeRow, onDeleteExercise }) {    
    return (
        <div className="w-full max-w-full">
            ...
            {/* ======================================= */}
            <div className="lg:hidden flex flex-col gap-4 w-full relative z-10">
                ...
                {data.exercises.map((ex, i) => (
                    <div key={ex.id || `mob-${i}`} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm relative overflow-visible">
                        ...
                        {/* PERBAIKAN: z-index dinamik menurun ke bawah (z-[50-i]) agar dropdown baris atas tidak tertutup oleh card baris bawahnya */}
                        <div className="mb-4 border border-slate-200 rounded-xl bg-slate-50 relative" style={{ zIndex: 50 - i }}>
                        ...
                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-2">
                            <input disabled={is_athlete} type="text" placeholder="Tambahkan catatan (opsional)..." value={ex.notes || ''} onChange={e => handleExChange(i, 'notes', e.target.value)} className="w-full text-xs p-2 bg-transparent outline-none italic text-slate-600 touch-manipulation placeholder-slate-400 min-w-0"/>
                        </div>
                    </div>
                ))}
            ...
            {/* ======================================= */}
            {/* VERSI DESKTOP (EXCEL VIEW)              */}
            {/* ======================================= */}
            
            {/* PERBAIKAN: Menambahkan class overflow-visible di parent agar dropdown tidak tersembunyi di dalam kotak tabel */}
            <div className="hidden lg:flex bg-white border border-slate-300 rounded-3xl shadow-sm relative z-20 flex-col w-full max-w-full overflow-visible">
                    ...
                    <table className="w-full min-w-[900px] table-fixed text-sm text-left border-collapse relative">
                        <thead className="bg-[#ff4d00] text-white text-[11px] font-bold uppercase tracking-wider relative z-50">
                            <tr>
                                ...
                                {!is_athlete && <th rowSpan="2" className="bg-orange-950 border-l border-b border-orange-700 px-1 py-3 text-center w-[5%] z-50"></th>}
                            </tr>
                            <tr className="bg-orange-900 text-[9px] relative z-40">
                        ...
                        <tbody className="relative z-30">
                            ...
                            {data.exercises.map((ex, i) => (
                                <tr key={ex.id || `desk-${i}`} className="hover:bg-orange-50/30 group bg-white border-b border-slate-200 last:border-b-0">
                                    ...
                                    {/* PERBAIKAN Z-INDEX ROW DINAMIS: Baris atas selalu menimpa baris bawah */}
                                    <td className="bg-white group-hover:bg-orange-50/50 border-r border-slate-200 p-0 relative" style={{ zIndex: 100 - i }}>
                                        <CreatableExerciseInput 
                                            value={ex.exercise_name} 
                                            options={libExercises} 
                                            disabled={is_athlete} 
                                    ...
                                    {!is_athlete && (
                                        <td className="bg-slate-50 group-hover:bg-orange-50/80 border-l border-slate-200 p-0 text-center relative z-10">
                                            <button type="button" onClick={() => removeRow(i)} className="text-rose-400 hover:text-rose-600 hover:bg-rose-100 p-4 w-full h-full min-h-[50px] flex items-center justify-center transition-colors mx-auto" title="Hapus Baris">
