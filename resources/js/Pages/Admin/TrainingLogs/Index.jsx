    ...
    return (
        <div ref={wrapperRef} className="relative w-full">
            <div
                ...
                className={`flex justify-between items-center w-full rounded-lg border ${isOpen ? 'border-[#00488b] ring-2 ring-[#00488b]/20 bg-white' : 'border-slate-200 bg-slate-50'} text-sm px-4 py-3.5 transition-all ${disabled ? 'cursor-not-allowed opacity-60 bg-slate-100' : 'cursor-pointer hover:border-slate-300'} ${className}`}
            >
            ...
            {isOpen && !disabled && (
                <div className="absolute top-[calc(100%+6px)] left-0 w-full bg-white border border-slate-200 shadow-xl rounded-lg max-h-56 overflow-y-auto z-[80] py-1.5 custom-scrollbar origin-top animate-in fade-in slide-in-from-top-2 duration-150">
                    {options.map((opt, idx) => {
    ...
    return (
        <AdminLayout title="Jadwal & Log Latihan">
            ...
            <div className="w-full max-w-[1400px] mx-auto min-w-0 pb-20">
                ...
                {/* HEADER UTAMA */}
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 w-full bg-white p-6 sm:p-8 rounded-lg border border-slate-200 shadow-sm relative overflow-hidden">
                    ...
                    <div className="min-w-0 flex-1 relative z-10">
                        ...
                        <h2 className="text-2xl md:text-3xl  font-bold text-slate-800 flex items-center gap-2 truncate tracking-tight">Jadwal Latihan</h2>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto shrink-0 relative z-10">
                        <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg p-1 shadow-inner w-full sm:w-auto justify-between">
                            <button onClick={() => navigateMonth('prev')} className="p-2 text-slate-500 hover:bg-white hover:text-[#00488b] hover:shadow-sm rounded-lg transition-all"><ChevronLeft className="w-5 h-5"/></button>
                            <div className="px-4  font-bold text-slate-700 w-36 text-center uppercase tracking-wider text-sm flex items-center justify-center gap-2"><CalendarDays className="w-4 h-4 text-slate-400"/> {monthName}</div>
                            <button onClick={() => navigateMonth('next')} className="p-2 text-slate-500 hover:bg-white hover:text-[#00488b] hover:shadow-sm rounded-lg transition-all"><ChevronRight className="w-5 h-5"/></button>
                        </div>
                                                        ...
                                                        <div className="px-6 py-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                                            ...
                                                            {!is_athlete && (
                                                                <div className="flex items-center gap-2 w-full md:w-auto mt-4 md:mt-0 border-t border-slate-200 md:border-0 pt-4 md:pt-0">
                                                                    <button onClick={() => openModal(item.date, item.session)} className="flex-1 md:flex-none px-4 py-2 text-xs font-bold text-amber-600 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Pencil className="w-3.5 h-3.5"/> Edit Info</button>
                                                                    <button onClick={() => setDeleteModal({ isOpen: true, id: item.session.id })} className="flex-1 md:flex-none px-4 py-2 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Trash2 className="w-3.5 h-3.5"/> Hapus</button>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
                    ...
                    <div className="bg-white w-full max-w-md rounded-lg shadow-2xl animate-in zoom-in-95 duration-200 my-auto flex flex-col">
                        ...
                        <form onSubmit={submit} className="p-6 space-y-5 rounded-b-3xl">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" value={form.data.date} onChange={e => form.setData('date', e.target.value)} className="w-full rounded-lg border border-slate-200 text-sm bg-slate-50 px-4 py-3 focus:ring-2 focus:ring-[#00488b]/20 focus:border-[#00488b] transition-all outline-none" required/>
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <input type="text" placeholder="Cth: Strength Training, Hipertrofi..." value={form.data.training_type} onChange={e => form.setData('training_type', e.target.value)} className="w-full rounded-lg border border-slate-200 text-sm bg-slate-50 px-4 py-3 focus:ring-2 focus:ring-[#00488b]/20 focus:border-[#00488b] transition-all outline-none font-medium" required/>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" placeholder="Cth: Apartemen / Gym Pusat..." value={form.data.location} onChange={e => form.setData('location', e.target.value)} className="w-full rounded-lg border border-slate-200 text-sm bg-slate-50 px-4 py-3 focus:ring-2 focus:ring-[#00488b]/20 focus:border-[#00488b] transition-all outline-none font-medium"/>
                            </div>
            ...
            {deleteModal.isOpen && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-sm rounded-lg shadow-2xl p-6 text-center animate-in zoom-in-95 duration-200">
                        ...
                        <h3 className="text-xl  font-bold text-slate-800 mb-2">Hapus Jadwal?</h3>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setDeleteModal({ isOpen: false, id: null })} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold text-sm rounded-lg hover:bg-slate-200 transition-colors">
                                Batal
                            </button>
                            <button onClick={confirmDelete} className="flex-1 py-3 bg-rose-500 text-white font-bold text-sm rounded-lg shadow-lg shadow-rose-500/30 hover:bg-rose-600 transition-colors">
                                Ya, Hapus!
                            </button>
                        </div>
                    </div>
