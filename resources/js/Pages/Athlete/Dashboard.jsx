...
import { 
    ...
    Target, Scale, Ruler, Weight, Clock, Zap, AlertCircle, ArrowRight
} from 'lucide-react';
import { 
...
export default function AthleteDashboard({ user, stats, radarData, history, trendData }) {

    // --- 1. LOGIC HELPER ---
    
    // Hitung BMI (dengan pembulatan 1 desimal)
    const calculateBMI = (h, w) => {
        if (!h || !w) return '-';
        const heightInM = h / 100;
        const bmiVal = w / (heightInM * heightInM);
        return parseFloat(bmiVal.toFixed(1)); // Dibulatkan 1 desimal
    };

    const bmi = calculateBMI(user.height, user.weight);
    const initial = user.name ? user.name.charAt(0).toUpperCase() : '-';

    // Analisis Otomatis: Cari Kekuatan (Top 3) & Kelemahan (Bottom 3)
        ...
        <AdminLayout title="My Dashboard">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col items-center text-center h-full relative overflow-hidden">
                    ...
                    <div className="relative z-10 w-24 h-24 bg-[#00488b] rounded-full flex items-center justify-center text-white text-3xl font-bold mb-4 shadow-xl shadow-blue-900/20 border-4 border-white">
                        {initial}
                    </div>
                    ...
                    <div className="flex gap-2 mb-6">
                        <span className="px-3 py-1 bg-blue-50 text-[#00488b] rounded-full text-xs font-bold  tracking-wider flex items-center gap-1">
                            <Trophy className="w-3 h-3" /> {stats.sport}
                        </span>
                        <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold  tracking-wider">
                            Athlete
                        </span>
                    </div>
                ...
                <div className="lg:col-span-2 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        ...
                        {/* Di sini otomatis dibulatkan oleh komponen StatCard di bawah */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
                        ...
                        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
                            <h3 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                                <span className="w-1.5 h-4 bg-[#00488b] rounded-full"></span>
                                Peta Kemampuan vs Target
                            </h3>
                            ...
                            {strengths.length > 0 ? strengths.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between group">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className="text-[10px] text-slate-400  tracking-wide">Avg Score</span>
                                    </div>
                                </div>
                            )) : (
                    ...
                    <div className="p-6 md:p-8">
                        ...
                        <div className="space-y-4">
                            {weaknesses.length > 0 ? weaknesses.map((item, idx) => (
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400">
                                                Target: <span className="font-bold">{item.B}</span>
                                            </p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                ...
                                <p className="text-slate-400 italic text-sm py-4">Tidak ada kelemahan signifikan atau data belum cukup.</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="px-6 py-5 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                    <h3 className="font-bold text-slate-700 flex items-center gap-2">
                        <Activity className="w-4 h-4 text-slate-400" /> Riwayat Latihan Terbaru
                    </h3>
                    <Link href={route('admin.performance.index')} className="text-xs font-bold text-[#00488b] hover:underline flex items-center gap-1">
                                ...
                                history.map((session) => (
                                    <tr key={session.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center text-slate-500 text-xs">
                                            {session.items_count} Tes
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            {/* Pembulatan skor di tabel history */}
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <Link 
                                                ...
                                                className="inline-flex items-center justify-center px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-600 hover:border-[#00488b] hover:text-[#00488b] transition-all"
                                            >
                                                Detail
                                            </Link>
                                        </td>
...
function StatCard({ label, value, icon: Icon, color, isText = false }) {
    ...
    // LOGIC PEMBULATAN DI SINI
    // Jika value adalah angka (bukan string kategori), kita bulatkan
    let displayValue = value;
    if (!isText && typeof value === 'number') {
        // Cek apakah angka desimal
        if (!Number.isInteger(value)) {
            displayValue = value.toFixed(1); // Bulatkan 1 angka belakang koma
        }
    }
    // Handle jika value datang sebagai string "66.666"
